...
class Coupon extends Public_Controller {
	...
	public function countrydata() {
		 $config = array();
        $config["base_url"] = base_url() . "coupon/countrydata";
        $config["total_rows"] = $this->coupon_model->record_count();
        $config["per_page"] = 5;
        $config["uri_segment"] = 3;
		$this->pagination->initialize($config);
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $data["results"] = $this->coupon_model->fetch_countries($config["per_page"], $page);
