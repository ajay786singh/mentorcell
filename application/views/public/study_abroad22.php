...
<h3>Filters</h3>
<div class="collegeFilterBox" style="display:block !important">
<?php $assigned_stream = $this->college_model->get_assigned_stream();
 ?>
<h4>Stream <span><!--<i class="icon-cw"></i> Reset--></span></h4>
<div class="filterItems" style="display:block !important">
<select name="checkstream" style="width:89%; display:block !important" id="checkstream">

<?php if(!empty($assigned_stream)){
foreach($assigned_stream as $a_stream){ 
$assigned_stream_name = $this->common_model->get_single_row('mc_streams','stream_id',$a_stream['stream_id']);	
if(!empty($assigned_stream_name['stream_name'])){
?>
<option <?php if($streamids==$a_stream['stream_id']){echo 'selected';} ?> value="<?=$a_stream['stream_id'];?>"><?=$assigned_stream_name['stream_name'];?> </option>
<?php }} } ?>
</select>
</div>
</div>
<div class="collegeFilterBox">
<h4>Courses <span><!--<i class="icon-cw"></i> Reset--></span></h4>
<div class="filterItems">
<select name="coursecheck" id="coursecheck" style="width: 86%;">

<?php $assigned_streamcourses = $this->college_model->get_assigned_streamcourses($streamids);
...
$assigned_course_name = $this->common_model->get_single_row('mc_courses','course_id',$a_streamcourse['course_id']);
 ?>
<option <?php if($coursename==$a_streamcourse['course_id']){echo 'selected';} ?> value="<?=$a_streamcourse['course_id'];?>"><?=$assigned_course_name['course_name'];?> </option>
<?php }?>
</select>
</div>

</div>
<div id="filteralldata">
<?php 
...
foreach($examnames as $examname){
	for($i=0;$i<count($examname);$i++){
			$ename = $this->college_model->get_exam_name($examname[$i]);
				if(!empty($ename)){
?>
<li><input type="checkbox" name="examcheck" value="<?=$examname[$i];?>" id="check1<?=$examname[$i]?>"> <label for="check1<?=$examname[$i]?>"><?=$ename->exam_name?></label></li>
		<?php 
 }
	}
} }
...
$specid = $specialdada['specialization_id'];
  ?>
    <li class="styleli"><a href="<?php echo base_url()."search?college=".$college->college_id."&stream=".$streamids['stream_id']."&type=".$college->course_id."&course_main=".$specid; ?>"><?=$special_data['specialization_name'];?></a></li>
  <?php } ?>
  </ul>
	...
	<?php }
		...
		<li class="catstyle" style="width:36%;"><span>Exam required</span>  <?php if(!empty($college->exam)){ $exam = explode(",",$college->exam);
...
var courseid = $(this).val();
    $.ajax({
			type: "GET",
			url: base_url+"index.php/home/study_abroad?course="+courseid,
			cache: false,
			beforeSend: function() {
              $('.loader').html('<img src="'+base_url+'assets/theme/images/bx_loader.gif" alt="" width="50" style="margin: 10% 0% 0% 30%;" >');
                                    },
		...
		$("#checkstream").change(function(){	
var streamid = $(this).val();
$.ajax({
				type: "GET",
				url: base_url+"index.php/home/streamcourses",
				dataType: 'text',
				data: {streamid:streamid},
				cache: false,
				success: function(res) {
					$("#coursecheck").html(res);
				}
			});			
		});
</script>
