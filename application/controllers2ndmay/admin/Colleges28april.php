...
class Colleges extends Admin_Controller {
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('conveyer_quota', 'Conveyer Quota', 'required');
		$this->form_validation->set_rules('management_quota', 'Management Quota', 'required');
		$this->form_validation->set_rules('international_quota', 'International Quota', 'required');
		...
		else
		{
			...
			$this->data['conveyer_quota']['value'] = $this->form_validation->set_value('conveyer_quota');
			$this->data['management_quota']['value'] = $this->form_validation->set_value('management_quota');
			$this->data['international_quota']['value'] = $this->form_validation->set_value('international_quota');
	...
	public function edit($id)
	{
		...
		if (isset($_POST) && ! empty($_POST))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$data['conveyer_quota'] = $this->input->post('conveyer_quota');
				$data['management_quota'] = $this->input->post('management_quota');
				$data['international_quota'] = $this->input->post('international_quota');
				$data['country'] = 101;
		...
		$this->data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
			...
			$this->data['conveyer_quota']['value'] = $this->form_validation->set_value('conveyer_quota',$colleges['conveyer_quota']);
			$this->data['management_quota']['value'] = $this->form_validation->set_value('management_quota',$colleges['management_quota']);
			$this->data['international_quota']['value'] = $this->form_validation->set_value('international_quota',$colleges['international_quota']);
