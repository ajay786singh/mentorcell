...
class Colleges extends Admin_Controller {
		...
		public function specialization()
	{
		...
		$option = '<select  class="form-control" required="" name="clg_specialization" id="clg_specialization" >
		<option value="">Select Specialization1</option>';
		foreach($specialization as $special){
			$option .= '<option value="'.$special['specialization_id'].'">'.$special['specialization_name'].'</option>';
		}
		$option .= '</select>';
		echo $option; die;
	}
