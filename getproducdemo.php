...
if(!empty($echeck)){
	$where .= "AND mc_course_assignment.exam LIKE '%$echeck%'";
}
if(!empty($loccheck)) {
	if(strstr($loccheck,',')) {
	...
	$where .= "AND mc_colleges.city = '$location'";
}
...
if(!empty($feecheck)) {
	...
	if($fee[0]==1200000 OR $fee[1]==1200000 OR $fee[2]==1200000 OR $fee[3]==1200000 OR $fee[4]==1200000 OR $fee[5]==1200000){
		...
		$fee = $feecheck;
		if($fee==99999){
		$where .= "AND mc_course_assignment.fee < '10000'";
	}
	
	if($fee==200000){
		$where .= "AND mc_course_assignment.fee BETWEEN '100000' AND '200000'";
	}
	
	if($fee==300000){
		$where .= "AND mc_course_assignment.fee BETWEEN '200000' AND '300000'";
	}
	
	if($fee==50000){
		$where .= "AND mc_course_assignment.fee BETWEEN '300000' AND '500000'";
	}
	
	if($fee==700000){
		$where .= "AND mc_course_assignment.fee BETWEEN '500000' AND '700000'";
	}
	
	if($fee==1200000){
		$where .= "AND mc_course_assignment.fee > '700000'";
	}
	}
...
if(!empty($speccheck)) {
	if(strstr($speccheck,',')) {
		...
		$speclizetion = $specarray;
	} else {
		$speclizetion = $speccheck;
	}
	$where .= "AND mc_course_assignment.specialization_id LIKE '%$speclizetion%'";
}
...
if(!empty($recogcheck)) {
	if(strstr($recogcheck,',')) {
		...
		$recognizetion = $recogarray;
	} else {
		$recognizetion = $recogcheck;
	}
	$where .= "AND mc_course_assignment.recognition LIKE '%$recognizetion%'";
}
	$sql = mysql_query("SELECT * FROM mc_colleges INNER JOIN mc_course_assignment ON mc_colleges.id = mc_course_assignment.college_id WHERE ".$where." GROUP BY mc_course_assignment.college_id");
